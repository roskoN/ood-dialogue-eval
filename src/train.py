...
assert (
    args.train_batch_size % args.gradient_accumulation_steps == 0
...
output_dir = join(
    args.output_dir,
    "GPT2.{}.{}.{}gpu.{}".format(
        args.learning_rate, args.train_batch_size, n_gpu, timestamp
    ),
)
log_dir = (
...
if args.fp16:
    ...
    if args.loss_scale == 0:
        ...
        optimizer = FP16_Optimizer(
            optimizer, static_loss_scale=args.loss_scale, verbose=False
        )
else:
    optimizer = Adam(
        optimizer_grouped_parameters, args.learning_rate, max_grad_norm=1.0
    )
            ...
            if global_step % args.valid_step == 0:
                if args.local_rank == -1 or get_rank() == 0:
                    ...
                    torch.save(
                        {
                            k: (
                                v.cpu() if v is not None else None
                            )  # save to cpu tensors
                    ...
                    logger.info(
                        "current learning rate: " + str(optimizer.param_groups[0]["lr"])
                    )
                    model.train()
