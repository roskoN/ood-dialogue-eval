...
from data_loader import (
    BucketingDataLoader,
    DistributedBucketingDataLoader,
    DynamicBatchingLoader,
)
from gpt2_training.distributed import all_gather_list, all_reduce_and_rescale_tensors
...
from lsp_model import (
    GPT2Config,
    GPT2LMHeadModel,
    GPT2LMHeadModelBackground,
    GPT2LMHeadModelOdin,
    GPT2LMHeadModelOdinFix,
    GPT2Tokenizer,
)
...
parser.add_argument("--init_checkpoint_regular", type=str)
parser.add_argument("--init_checkpoint_background", type=str)
...
output_dir = join(
    args.output_dir, "{}_llr".format(args.eval_input_file.split("/")[-1],),
)
log_dir = (
...
model_regular = load_model(
    GPT2LMHeadModel(config), args.init_checkpoint_regular, args, verbose=True
)
model_regular = model_regular.eval()

model_background = load_model(
    GPT2LMHeadModelBackground(config),
    args.init_checkpoint_background,
    args,
    verbose=True,
)
model_background = model_background.eval()
...
with torch.no_grad():
    for item_idx, batch in enumerate(tqdm(eval_dataloader_loss)):
        try:
            ...
            if args.no_token_id:
                token_ids = None
            loss_reg, _, lm_logits_reg = model_regular(
                input_ids, position_ids, token_ids, label_ids
            )
            _, _, lm_logits_back = model_background(
                input_ids, position_ids, token_ids, label_ids
            )
            losses[item_idx] = loss_reg.detach().cpu().squeeze().numpy()
            lm_max[item_idx] = (
                torch.softmax(lm_logits_reg.detach().squeeze(), dim=-1)
                .cpu()
                .max(dim=-1)[0]
                .numpy()
            )

            label_ids = label_ids.detach().cpu().squeeze()

            lm_logits_reg = torch.log_softmax(
                lm_logits_reg.detach().cpu().squeeze(), dim=-1
            )
            argmax_reg = torch.stack(
                [logit[label_id] for label_id, logit in zip(label_ids, lm_logits_reg)]
            )

            lm_logits_back = torch.log_softmax(
                lm_logits_back.detach().cpu().squeeze(), dim=-1
            )
            argmax_back = torch.stack(
                [logit[label_id] for label_id, logit in zip(label_ids, lm_logits_back)]
            )

            lm_argmax[item_idx] = argmax_reg.cpu().numpy()
