...
class GPT2FeatureDataset(Dataset):
    ...
    def collate(features):
        ...
        position_ids = pad_sequence(
            [torch.tensor(f.position_ids, dtype=torch.long) for f in features],
            batch_first=True,
            padding_value=0,
        )
        token_type_ids = pad_sequence(
            [torch.tensor(f.token_type_ids, dtype=torch.long) for f in features],
            batch_first=True,
            padding_value=0,
        )
        labels = pad_sequence(
...
class DynamicBatchingLoader(object):
    ...
    def __init__(
        self, corpus_file, tokenizer, normalize_data, batch_size, max_seq_length
    ):
    ...
    def get_len(self, corpus):
        n_line = int(
            sp.check_output(f"wc -l {corpus}".split(), universal_newlines=True).split()[
                0
            ]
        )
        return n_line
